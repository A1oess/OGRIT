...
OCCLUSION_LINE_LENGTH = 100

# Set up the colors for the plots.
color_map = {"green":"#9bbb59", "light_blue":"#4bacc6", "blue":"#4f81bd", "red":"#c0504d", "purple":"#8064a2"}
OCCLUSIONS_COLOR = color_map["light_blue"]
OCCLUDED_LANE_COLOR = color_map["blue"]
OBSTACLES_COLOR = color_map["green"]

OCCLUDED_AREA_ALPHA = 0.5
OCCLUSION_SEGMENTS_ALPHA = 1.0
...
class OcclusionDetector2D:
    ...
    def __init__(self, scenario_name: str, episode_idx: int, debug: bool = False, debug_steps: bool = False):
        ...
        self.scenario_map = Map.parse_from_opendrive(f"/home/developer/workspace/zhaoxh/OGRIT/scenarios/maps/{self.scenario_name}.xodr")
        self.scenario_config = ScenarioConfig.load(f"/home/developer/workspace/zhaoxh/OGRIT/scenarios/configs/{self.scenario_name}.json")
    ...
    def get_occlusions_frame(self, frame):
        ...
        for ego_idx, (ego_id, ego_vehicle) in enumerate(vehicles_in_frame):
            ...
            if self.debug:
                self.plot_map(frame=frame, obstacles=obstacles, ego=[ego_vehicle_boundary])
                self.plot_occlusions(ego_position, self.occlusion_lines, ego_occluded_lanes)
                self.occlusion_lines = []
                
            frame_occlusions[ego_id] = ego_occluded_lanes
        plt.savefig(f"{self.scenario_name}_occlusion.png", bbox_inches='tight')
    ...
    def get_occlusions_ego(self, ego_position, obstacles, ego_vehicle_boundary):
        ...
        for u in obstacles:
            ...
            if self.debug_steps:
                self.plot_map(obstacles=obstacles, ego=[ego_vehicle_boundary])
    ...
    def plot_map(self, frame: Dict[int, AgentState] = None, obstacles: List[List[List[float]]] = None, ego: List[List[List[float]]] = None):
        ...
        if self.scenario_config is not None:
            ip.plot_map(self.scenario_map, markings=False, midline=False, scenario_config=self.scenario_config,
                        plot_background=True, ignore_roads=True, plot_goals=False)
        else:
        ...
        if obstacles:
            for obstacle in obstacles:
                ...
                OcclusionDetector2D.plot_area(x, y, alpha=0.7, color=OBSTACLES_COLOR, contour=True)
        if ego:
            for ego_veh in ego:
                x, y = list(zip(*ego_veh))
                OcclusionDetector2D.plot_area(x, y, alpha=1.0, color=color_map["red"], contour=True)
    ...
    def plot_occlusions(ego_position: np.array, occlusion_lines: List[List[Tuple[int, int]]],
        ...
        for occlusion_line in occlusion_lines:
            ...
            plt.plot([x0, x3], [y0, y3], "--", linewidth=0.8, color=OCCLUSIONS_COLOR, alpha=0.5)
            plt.plot([x0, x4], [y0, y4], "--", linewidth=0.8, color=OCCLUSIONS_COLOR, alpha=0.5)
