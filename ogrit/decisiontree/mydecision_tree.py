...
class myNode:
    ...
    @classmethod
    def fit(cls, samples: pd.DataFrame, observable_flag: str, goal: Union[int, str], alpha=0, min_samples_leaf=1, max_depth=None,
            ccp_alpha=0.):
        possible_goal = samples.goal_type if isinstance(goal, str) else samples.possible_goal
        samples['has_goal'] = samples.possible_goal == samples.true_goal
        goal_training_samples = samples.loc[possible_goal == goal]
        ...
        def _recursive_split(node: myNode, node_samples: pd.DataFrame, true_indicators, false_indicators):
            ...
            if (node_samples.has_goal.nunique() != 1
                ...
                elif observable_flag == 'observed':
                    allowed_features = list(base_features)
