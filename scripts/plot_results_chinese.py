...
model_names = ['GOIT', 'occlusion_baseline', 'generalised_grit','occlusion_grit'] # ['GOIT', 'occlusion_baseline','occlusion_grit', 'generalised_grit']
...
for scenario_idx, scenario_name in enumerate(scenario_names):
    ...
    for model_name in model_names:
        if model_name != 'uniform_prior_baseline':
            accuracy_sem = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_acc_sem.csv')
            accuracy = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_acc.csv')
...
for scenario_idx, scenario_name in enumerate(scenario_names):
    ...
    if scenario_idx % 2 == 1:
        plt.xlabel('观测轨迹完成率')
    plt.title('归一化熵 ({})'.format(scenario_name))
    marker = itertools.cycle(('^', '+', 'x', 'o', '*'))

    for model_name in model_names:
        entropy_norm_sem = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_entropy_norm_sem.csv')
        entropy_norm = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_entropy_norm.csv')
    ...
    plt.legend()
    plt.subplots_adjust(wspace =0.2, hspace =0.5)#调整子图间距
plt.savefig('/home/goose/zhaoxh/OGRIT' + '/images/归一化熵_ogrit.pdf', bbox_inches='tight')
...
for scenario_idx, scenario_name in enumerate(scenario_names):
    ...
    for model_name in model_names:
        true_goal_prob_sem = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_true_goal_prob_sem.csv')
        true_goal_prob = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_true_goal_prob.csv')
...
for scenario_idx, scenario_name in enumerate(scenario_names):
    ...
    if scenario_idx % 2 == 1:
        plt.xlabel('观测轨迹完成率')
    plt.title('交叉熵 ({})'.format(scenario_name))
    marker = itertools.cycle(('^', '+', 'x', 'o', '*'))

    for model_name in model_names:
        cross_entropy_sem = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_cross_entropy_sem.csv')
        cross_entropy = pd.read_csv('/home/goose/zhaoxh/OGRIT' + f'/results/{scenario_name}_{model_name}_cross_entropy.csv')
    ...
    plt.legend()
    plt.subplots_adjust(wspace =0.2, hspace =0.5)#调整子图间距
plt.savefig('/home/goose/zhaoxh/OGRIT' + '/images/交叉熵_ogrit.pdf', bbox_inches='tight')
